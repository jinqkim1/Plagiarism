...
public class WholeFileRecordReader extends RecordReader<NullWritable, Text> {
    ...
    //  private final Text mFileName;
      ...
      public WholeFileRecordReader(CombineFileSplit fileSplit, TaskAttemptContext context,
    	  ...
    	  DBManager.getInstance().insertSQL("insert into `plagiarismdb`.`workflow` (`type`) value ('WholeFileRecordReader')");
    	 
    	  FileSplit fileSplit1;
		try {
			fileSplit1 = new FileSplit(fileSplit.getPath(pathToProcess), fileSplit.getOffset(pathToProcess), fileSplit.getLength(pathToProcess), fileSplit.getLocations());
			String currentPath = fileSplit1.getPath().toString();
			 DBManager.getInstance().insertSQL("insert into `plagiarismdb`.`workflow` (`type`) value ('currentPath : "+currentPath+"')");
	    	 
		} catch (IOException e) {
			// TODO Auto-generated catch block
			DBManager.getInstance().insertSQL("insert into `plagiarismdb`.`workflow` (`type`) value ('currentPath : "+e.toString()+"')");
