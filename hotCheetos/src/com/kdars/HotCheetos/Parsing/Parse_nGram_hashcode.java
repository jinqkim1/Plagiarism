...
public class Parse_nGram_hashcode implements Parse{
	...
	public boolean parseDocJINKYU(String content, int documentID) {
		DocumentInfo docInfo = new DocumentInfo();
		docInfo.docID = documentID;
		...
		return DBManager.getInstance().insertBulkToHashTable(docInfo);
	}
	
	public boolean parseDocSetWithDocIDArrayJINKYU(ArrayList<Integer> docIDs) {
		boolean jobCompleteChecker = true;
		
		for (int docid : docIDs){
			String content = DBManager.getInstance().getText(docid);
			if (!parseDocJINKYU(content, docid)){
	...
	public ArrayList<DocumentInfo> getParsedDocs(ArrayList<Integer> docIDs){
		return DBManager.getInstance().getDocInfoArray(docIDs);
	}
